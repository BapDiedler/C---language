...
let rec get_ast_typ ptyp parent_id = 
  ...
  match ptyp with
  | Tscalar btyp -> create_node "Tscalar" [] [ ("btyp", get_ast_typ_basic btyp) ]
  | Tfun p -> create_node "Tfun" [] [
      ("fun", fold_children get_ast_typ_basic (List.append (p.args) [p.result]) )
    ]
and get_ast_typ_basic btyp parent_id =
  let create_node = create_node parent_id in
  match btyp with
  | Tint -> create_node "Tint" [] []
  | Tptr t -> create_node "Tptr" [] [ ("ptyp", get_ast_typ t) ]
...
and get_ast_ptyp ptyp parent_id =
  ...
  | TFUN (res,args) ->
    create_node "TFUN" [] [
      ("args", fold_children get_ast_ptyp args);
      ("res", get_ast_ptyp res)
    ]
