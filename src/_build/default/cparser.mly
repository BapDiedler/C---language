...
constant : CONSTANT { $sloc, $1 };

identifier  : IDENTIFIER      { $sloc, $1 };
open_brace  : OPEN_BRACE_CHR  { $sloc };
close_brace : CLOSE_BRACE_CHR { $sloc };

string_literal:
          STRING_LITERAL { $sloc, $1 }
...
add_chr     : ADD_CHR   { $sloc, ADD_CHR   }
sub_chr     : SUB_CHR   { $sloc, SUB_CHR   }
bang_chr    : BANG_CHR  { $sloc, BANG_CHR  }
tilde_chr   : TILDE_CHR { $sloc, TILDE_CHR }
star_chr    : STAR_CHR  { $sloc, STAR_CHR  }
amp_chr     : AMP_CHR   { $sloc, AMP_CHR   }

close_paren : CLOSE_PAREN_CHR { $sloc }
...
declaration_ptr_fun:
        ...
        { let loc,var = $4 in CDECL(loc,var,TFUN($1,$6)) }
        ;
...
type_specifier: scalar_specifier
            { $1 }
        ;

parameter_specifier: 
          OPEN_PAREN_CHR CLOSE_PAREN_CHR { [] }
        | OPEN_PAREN_CHR parameter_specifier_list CLOSE_PAREN_CHR { $2 }
        ;

parameter_specifier_list:
          type_specifier_list { List.rev $1 }
        ;
